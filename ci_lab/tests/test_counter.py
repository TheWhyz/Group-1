...
class TestCounterEndpoints:
    ...
    def test_bottom_n_counters(self, client):
        ...
        data = response.get_json()
        assert 'b' in data, f"Expected counter 'b' to be in the bottom N counters, but got {data}"

    # ===========================
    # Test: Set a counter to a specific value
    # Author: Student 4 - Matthew Rainwater
    ...
    def test_set_counter_to_value(self, client):
        ...
        response_same = client.put('/counters/test1/set/5')
        assert response_same.status_code == HTTPStatus.OK
        assert response_same.get_json() == {"test1": 5} 

    # ===========================
    # Test: Prevent negative counter values
    # Author: Student 5 - Joseph Dib
    ...
    def test_reset_single_counter(self, client):
        ...
        assert response.status_code == HTTPStatus.OK
        assert response.get_json() == {"test1": 0}, "Counter should still exist and be retrievable after reset"

    # ===========================
    # Test: Prevent resetting a non-existent counter
    # Author: Student 7 - Rubi Escobedo
    ...
    def test_prevent_resetting_non_existent_counter(self, client):
        ...
        assert 'not found' in response.get_json().get('error', '').lower()  # Verify error message contains 'not found'
    ...
    def test_get_total_number_of_counters(self, client):
        ...
        assert response.get_json()["count"] == 2  # make count exact value 2
